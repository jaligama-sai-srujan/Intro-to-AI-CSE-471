...
class GradientDescentSolver(Solver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        print(loss_tensor,update_ops)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: train_data[0],target_ph: train_data[1]})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        batch_train = tfu.MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        batch_val = tfu.MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ...
        for iter_ in xrange(self.iterations):
            ...
            batch_training = batch_train.next()
            batch_value = batch_val.next()
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_training[0], placeholders[1]: batch_training[1]})
            session.run(update_ops, feed_dict={placeholders[0]: batch_training[0], placeholders[1]: batch_training[1]})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_value[0], placeholders[1]: batch_value[1]})
            train_losses.append(train_loss)
            val_losses.append(val_loss)
            if callback is not None: callback(model)
            self.display_progress(iter_, train_losses, val_losses)
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            # val_loss should be the loss of this iteration using the same amount of data used for the updates, but using the validation data instead
            batch_training = trainGenerator.next()
            val_training = valGenerator.next()
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_training[0], placeholders[1]: batch_training[1]})
            session.run(update_ops, feed_dict={placeholders[0]: batch_training[0], placeholders[1]: batch_training[1]})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: val_training[0], placeholders[1]: val_training[1]})
